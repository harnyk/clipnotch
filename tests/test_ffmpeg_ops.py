...
import pytest
from clipnotch.ffmpeg_ops import (
...
def test_run_ffmpeg_raises_on_failure():
    ...
    with patch("clipnotch.ffmpeg_ops.subprocess.run", return_value=fake_result):
...
def test_check_ffmpeg_available_true_when_found():
    with patch("clipnotch.ffmpeg_ops.shutil.which", return_value="/usr/bin/ffmpeg"):
        assert check_ffmpeg_available() is True


def test_check_ffmpeg_available_false_when_missing():
    with patch("clipnotch.ffmpeg_ops.shutil.which", return_value=None):
